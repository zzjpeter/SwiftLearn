...
//3.定义多个不同类型的泛型,则需要在尖括号中加入多个节点:<T,U,V> ;

//你可以对节点进行一些限制, 比如要求泛类型遵守某些协议, Swift中数组的判等就是这样定义的:
//public func ==<Element : Equatable>(lhs: [Element], rhs: [Element]) ->Bool

//Element 是节点声明的, 它代表一个泛型, 可以看到这里的泛型名是Element, 相比上面的"T","U","V"长的多. 这是因为此处的 Element不仅仅是一个占位符的作用, 它还声明了这个泛型代表数组中的元素类型, 有具体的意义, 这种 API 的设计值的我们借鉴;
...
//注意:如果你尝试在实现时"嵌套"一个泛型,那么会导致泛型无法被特化; 比如数组本身是泛型的,在声明数组类型时传入了另一个泛型, 那么你将无法初始化该数组:
struct TestStruct3< T:Comparable >{
    var array:[T] // as! [T]  这样貌似就能解决了, 但是这样泛型就没有意义了
    init(array: [T]) {
        self.array = array
    }
}

var testStruct3 = TestStruct3<Int>(array: [1, 2, 3, 4])
print(testStruct3)
